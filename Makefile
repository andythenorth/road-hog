...
ifeq ($(strip $(EXPORTED)),no)
  ...
  # we pick the 5th item from git info for Hog, as Hog uses a monorepo strategy and tags/versions need special handling
...
# result is e.g. grfcodec -s -e -c -g 2 road-hog.grf generated/
