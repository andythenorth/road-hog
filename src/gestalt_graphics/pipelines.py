...
class Pipeline(object):
    ...
    def vehicle_source_input_path(self):
        ...
        if hasattr(self, "vehicle_unit"):
            if self.vehicle_unit.base_platform is not None:
                spritesheet_name = self.vehicle_unit.base_platform.get_spritesheet_name_body_or_complete_vehicle(
                    self.consist
                )
    ...
    def process_buy_menu_sprite(self, spritesheet):
        ...
        for counter, unit in enumerate(self.consist.units):
            ...
            if unit.unit_is_semi_tractor:
                unit_length_in_pixels = 6 * (
                    unit.vehicle_length + unit.semi_truck_shift_offset_jank
                )  # !! hax
            ...
            unit_spriterow_offset = (
                unit.spriterow_num * graphics_constants.spriterow_height
            )
            crop_box_src = (
                227,
            ...
            buy_menu_sprite_mask = buy_menu_sprite_mask.point(
                lambda i: 0 if i == 0 else 255
            ).convert(
                "1"
            )  # the inversion here of blue and white looks a bit odd, but potato / potato
            buy_menu_image.paste(
                custom_buy_menu_sprite, crop_box_dest, buy_menu_sprite_mask
            )
    ...
    def render_common(self, input_image, units):
        ...
        output_path = os.path.join(
            currentdir, "generated", "graphics", self.consist.id + ".png"
        )
...
class ExtendSpriterowsForCompositedSpritesPipeline(Pipeline):
    ...
    def add_generic_spriterow(self):
        ...
        vehicle_generic_spriterow_input_image = self.comp_chassis_and_body(
            self.vehicle_unit_source_image.copy().crop(crop_box_source)
        )
    ...
    def add_livery_only_spriterows(self, recolour_map):
        ...
        vehicle_livery_only_spriterow_input_image = self.comp_chassis_and_body(
            self.vehicle_unit_source_image.copy().crop(crop_box_source)
        )
        # vehicle_generic_spriterow_input_image.show() # comment in to see the image when debugging
        vehicle_livery_only_spriterow_input_as_spritesheet = (
            self.make_spritesheet_from_image(vehicle_livery_only_spriterow_input_image)
        )
        ...
        for label, recolour_map in self.consist.gestalt_graphics.recolour_maps:
            ...
            self.units.append(
                AppendToSpritesheet(
                    vehicle_livery_only_spriterow_input_as_spritesheet, crop_box_dest
                )
            )
    ...
    def add_bulk_cargo_spriterows(self):
        ...
        vehicle_bulk_cargo_input_image_1 = self.comp_chassis_and_body(
            self.vehicle_unit_source_image.copy().crop(crop_box_source_1)
        )
        vehicle_bulk_cargo_input_image_2 = self.comp_chassis_and_body(
            self.vehicle_unit_source_image.copy().crop(crop_box_source_2)
        )
        ...
        bulk_cargo_rows_image.paste(
            vehicle_bulk_cargo_input_image_1, crop_box_comp_dest_1
        )
        bulk_cargo_rows_image.paste(
            vehicle_bulk_cargo_input_image_2, crop_box_comp_dest_2
        )

        bulk_cargo_rows_image_as_spritesheet = self.make_spritesheet_from_image(
            bulk_cargo_rows_image
        )
    ...
    def add_piece_cargo_spriterows(self):
        ...
        vehicle_cargo_loc_image = self.vehicle_unit_source_image.copy().crop(
            crop_box_vehicle_cargo_loc_row
        )
        # get the loc points
        loc_points = [
        ...
        vehicle_base_image = self.comp_chassis_and_body(
            self.vehicle_unit_source_image.copy().crop(crop_box_vehicle_body)
        )
        ...
        vehicle_mask_source = (
            self.vehicle_unit_source_image.copy()
        ...
        for (
            cargo_labels,
            cargo_filenames,
        ) in self.consist.gestalt_graphics.piece_cargo_maps:
            for cargo_filename in cargo_filenames:
                ...
                cargo_sprites = self.get_arbitrary_angles(
                    cargo_sprites_input_image,
                    cargo_spritesheet_bounding_boxes[self.vehicle_unit.cargo_length],
                )
    ...
    def render(self, consist, global_constants, graphics_output_path):
        ...
        for vehicle_counter, vehicle_rows in enumerate(
            consist.get_spriterows_for_consist_or_subpart(consist.unique_units)
        ):
            ...
            vehicle_unit_source_base_yoffs = 10 + (
                graphics_constants.spriterow_height
                * consist_cumulative_source_spriterow_count
            )
            ...
            for spriterow_data in vehicle_rows:
                ...
                if "base_platforms" in self.vehicle_source_input_path:
                    self.vehicle_unit_source_row_yoffs = 10 + (
                        vehicle_unit_cumulative_source_spriterow_count
                        * graphics_constants.spriterow_height
                    )
                ...
                if (
                    ...
                    self.add_livery_only_spriterows(
                        consist.gestalt_graphics.recolour_maps
                    )
