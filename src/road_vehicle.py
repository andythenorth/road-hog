...
class Consist(object):
    ...
    def __init__(self, **kwargs):
        ...
        self._name = kwargs.get(
            "name", None
        )  # private as 'name' is an @property method to add type substring
        ...
        if self.road_type is not None and self.tram_type is not None:
            utils.echo_message(
                "Error: "
                + self.id
                + ". Vehicles must not have both road_type and tram_type properties set.  Set one of these only"
            )
        ...
        self.requires_electricity_supply = (
    ...
    def add_unit(self, repeat=1, **kwargs):
        ...
        assert base_platform != False, (
            "%s unit %s is missing base_platform kwarg; set it to a base platform id, or 'None'"
            % (self.id, count)
        )
        ...
        if base_platform is not None:
            base_platform = (
                base_platform()
            )  # init the base_platform, so we have an instance, not a class name
    ...
    def name_type_suffix(self):
        ...
        return (
            "STR_NAME_SUFFIX_"
            + self.name_suffix_consist_type
            + self.name_suffix_vehicle_type
        )
    ...
    def power_type_suffix(self):
        if self._power_type_suffix is None:
            utils.echo_message(
                "Consist "
                + self.id
                + " has no _power_type_suffix set by any of its units"
            )
        else:
    ...
    def name(self):
        return (
            "string(STR_NAME_CONSIST, string(STR_NAME_"
            + self.id.upper()
            + "), string("
            + self.name_type_suffix
            + "), string("
            + self.power_type_suffix
            + "))"
        )
    ...
    def get_engine_cost_points(self):
        ...
        if self.power > 2000:
            utils.echo_message(
                "Consist " + self.id + " has power > 2000hp, which is too much"
            )
        ...
        if self.speed > 90:
            utils.echo_message(
                "Consist " + self.id + " has speed > 90, which is too much"
            )
        ...
        if self.intro_date > 2030:
            utils.echo_message(
                "Consist " + self.id + " has intro_date > 2030, which is too much"
            )
        ...
        if self.total_capacity > 255:
            utils.echo_message(
                "Consist " + self.id + " has capacity > 255, which is too much"
            )
    ...
    def gen(self):
        ...
        if self._gen:
            ...
            for gen_counter, intro_date in enumerate(
                self.roster.intro_dates[self.base_track_type_name]
            ):
    ...
    def buy_menu_width(self):
        ...
        if consist_length < global_constants.docs_sprite_max_width:
            return consist_length
        else:
            return global_constants.docs_sprite_max_width
...
class TrackTypeMixinBase(object):
    ...
    * name_suffix_vehicle_type, use to derive 'Tram', 'Truck' etc in name strings
    ...
    base_track_type_name = (
        None  # set this in subclass as a label; fail if not set explicitly
    )
    roadveh_flag_tram = False
    tractive_effort_coefficient = (
        None  # set in subclass to float (0..1); fail if not set explicitly
    )
    name_suffix_vehicle_type = (
        None  # set in subclass to string; fail if not set explicitly
    )
...
class TrackTypeMixinTruckBusCoach(TrackTypeMixinBase):
    ...
    tractive_effort_coefficient = 0.7
    name_suffix_vehicle_type = (
        "_TRUCK"  # default to truck, over-ride in consist subclasses for bus or coach
    )
...
class CoveredHopperTruckConsist(
    CoveredHopperHaulerConsistBase, TrackTypeMixinTruckBusCoach
):
    """
...
class LogHaulerConsistBase(Consist):
    ...
    def __init__(self, **kwargs):
        ...
        self.gestalt_graphics = GestaltGraphicsCustom(
            {"WOOD": [0]}, "vehicle_with_visible_cargo.pynml", generic_rows=[0]
        )
...
class RefrigeratedTruckConsist(
    RefrigeratedHaulerConsistBase, TrackTypeMixinTruckBusCoach
):
    """
    Refrigerated truck.
    """
...
class RoadVehicle(object):
    ...
    def __init__(self, **kwargs):
        ...
        self.numeric_id = kwargs.get(
            "numeric_id"
        )  # this is required, fail if not passed
        ...
        self.semi_truck_shift_offset_jank = kwargs.get(
            "semi_truck_shift_offset_jank", None
        )
    ...
    def capacity(self):
        if self._capacity is not None:
            ...
            base_capacity = self.consist.roster.unit_capacity_per_vehicle_type[
                self.consist.vehicle_role
            ][self.consist.gen - 1]
            ...
            if self.unit_position_in_consist == 0:
                if self._capacity != None:
                    ...
                    utils.echo_message(
                        "Error: "
                        + self.id
                        + ".  First unit of semi-truck must not have explicit capacity set"
                    )
    ...
    def get_loading_speed(self, cargo_type, capacity_multiplier):
        ...
        result = int(
            self.consist.loading_speed_multiplier
            * math.ceil(capacity / transport_type_rate)
        )
        return max(result, 1)
    ...
    def offsets(self):
        if self.semi_truck_shift_offset_jank:
            result = []
            for i in range(0, 8):
                base_offsets = global_constants.default_road_vehicle_offsets[
                    str(self.vehicle_length)
                ][i]
                offset_deltas = [
                    self.semi_truck_shift_offset_jank * offset
                    for offset in global_constants.semi_truck_offset_jank[i]
                ]
                result.append(
                    [
                        base_offsets[0] + offset_deltas[0],
                        base_offsets[1] + offset_deltas[1],
                    ]
                )
            return result
        else:
            return global_constants.default_road_vehicle_offsets[
                str(self.vehicle_length)
            ]
    ...
    def spriterow_num(self):
        ...
        preceding_spriterows = self.consist.get_spriterows_for_consist_or_subpart(
            self.consist.units[0 : self.consist.units.index(self)]
        )
        result = []
    ...
    def get_effects(self):
        ...
        if len(self.effects) > 0:
            for index, effect in enumerate(self.effects):
                result.append(
                    "STORE_TEMP(create_effect(" + effect + "), 0x10" + str(index) + ")"
                )
        elif self.default_effect_sprite is not None:
            result.append(
                "STORE_TEMP(create_effect("
                + self.default_effect_sprite
                + ", -2, 0, 10), 0x100)"
            )
