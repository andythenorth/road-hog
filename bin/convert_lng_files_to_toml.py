# Road Hog converted lang sources from .lng files to .toml in September 2023
...
def write_toml(lang_data, lang_name, toml_dst):
    ...
    for lang_string_name, lang_string in lang_data["lang_strings"].items():
        # this might not handle string-local pragmas for case or gender??
...
def main():
    ...
    # don't walk all the rosters, as there's only one of use
    for file_name in os.listdir(os.path.join("src", "lang", "landrace")):
        if file_name != ".DS_Store":
            lng_file = codecs.open(
                os.path.join("src", "lang", "landrace", file_name),
                "r",
                encoding="utf-8",
            )
